#This module worked by - Sony Jenith
...
class MachineLearning():
    ...
    def __init__(self):
        ...
        # Replace '.' with '' in specified columns - ensure these columns are treated as strings before replacement
        self.flow_dataset.iloc[:, 2] = self.flow_dataset.iloc[:, 2].astype(str).str.replace('.', '')
        self.flow_dataset.iloc[:, 3] = self.flow_dataset.iloc[:, 3].astype(str).str.replace('.', '')
        self.flow_dataset.iloc[:, 5] = self.flow_dataset.iloc[:, 5].astype(str).str.replace('.', '')

        # Convert columns to numeric, coercing errors to NaN
        for col in [2, 3, 5]:
            self.flow_dataset.iloc[:, col] = pd.to_numeric(self.flow_dataset.iloc[:, col], errors='coerce')
